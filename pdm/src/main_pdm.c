...
//     // input array.
//     for (int tau = 0; tau < N_R; tau ++){
//         for (int i = 0; i < N/2 ; i ++){
//             R[tau] +=
//(((int16_t)pcm_buf[2*i]*(int16_t)pcm_buf[(2*(i-tau)+N_R)%N]) );
//         }
//         R[tau] /= (N/2);
//     }
//}
...
//Fonction pour calculer la corr̩lation crois̩e des ̩chantillons PCM
//void compute_xcorr(uint16_t *pcm_buf, int16_t N, float *R, int N_R) {
//    for (int tau = 0; tau < N_R; tau++) {
//        R[tau] = 0;
//        for (int i = 0; i < N / 2; i++) {
//            R[tau] += (int16_t)pcm_buf[2 * i] * (int16_t)pcm_buf[(2 * (i - tau) + N_R) % N];
//        }
//        R[tau] /= (N / 2);
//    }
//}
...
int main(void)
{
    ...
    // Convert theta to degrees for better interpretation
                ...
                /*char message[50];
                sprintf(message, "l'angle est de %d\r\n", angle_deg); */   
                /*float time_delay = indice/SAMPLING_FREQ;

                // Afficher les r̩sultats pour v̩rification
                float tau = tau_array[indice];
			    float x = 340.0 * time_delay/distance;
			    int angle_deg;

			    if (x > 1) {
			        float asinh_x = asinhf(x);
			        float asinh_1 = asinhf(1.0f);
			        float ramene_x = asinh_x / asinh_1;
			        angle_deg = ramene_x * 180.0 / pi;
			    } else if (x < -1) {
			        float asinh_x = asinhf(-x);
			        float asinh_1 = asinhf(1.0f);
			        float ramene_x = asinh_x / asinh_1;
			        ...
			        float angle = asinf(x);
			        angle_deg = (int)(angle * 180.0 / pi);
			    }*/


    			uart_printf(_USART2, "l'angle est de %d \r\n",angle_deg);
