...
#DOA
def get_yaw_angle():
    if ser_mic.in_waiting > 0:      # Vérifie s'il y a des données disponibles à lire
        char = ser_mic.read_until(b'X')
        char = char[:-1]
        return char
...
class ManualModeWindow(tk.Toplevel):
    ...
    def valider(self):
        ...
        print(serialised)
        ser_motor.write(serialised.encode())
        ser_motor.close()
    ...
    def __init__(self, master):
        ...
        while True:
            ...
            if iterateur%5 == 0:
                ...
                yaw = get_yaw_angle()
                serialised = convertforserial(mead, yaw)
                print(serialised)
                ser_motor.write(serialised.encode()) #envoyer la donnée à la carte de commande de moteurs
                time.sleep(0.1)
                tableau = []
                iterateur = 0
                
            else:
                pitch = int(get_y_angle(nearest_face[0], screen_center)/2)
                if pitch == 45:
                    pitch = 0
                tableau.append(pitch/2)
